...
def evaluate_batch_gpu(ind_lists):
    ...
    return formable_counts.cpu().numpy()
...
def main():
    ...
    # 초기 평가 및 max_count_all_time 설정
    counts = evaluate_batch_gpu([list(ind) for ind in pop])
    for ind, c in zip(pop, counts): ind.fitness.values = (c,)
    
    max_count_all_time = max([ind.fitness.values[0] for ind in pop]) if pop else -1.0
    print(f"Initial Max Count: {max_count_all_time}")
    ...
    for g in range(1, 10001):
        ...
        if invalid_ind:
            i_counts = evaluate_batch_gpu([list(ind) for ind in invalid_ind])
            for ind, c in zip(invalid_ind, i_counts): ind.fitness.values = (c,)

        pop = update_crowding(pop + offspring)[:TARGET_POP]
        current_max = max([ind.fitness.values[0] for ind in pop])
        
        if current_max > max_count_all_time:
            print(f"⚡ UPDATE! Gen {g}: {max_count_all_time} -> {current_max}")
            max_count_all_time = current_max
            best_ind = copy.deepcopy(pop[np.argmax([ind.fitness.values[0] for ind in pop])])
            improved = True

        if g % 500 == 0:
            print(f"Gen {g:3d} | Max: {max_count_all_time}")
  ...
  NO PROGRESS IN THIS RUN!
    ...
    if best_ind and improved:
        ...
        print(f"Final Success: {max_count_all_time} saved.")

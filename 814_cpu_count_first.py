...
@njit(fastmath=True)
def _dlas_count_core_numba(initial_grid, initial_count, max_idle, rows, cols):
    """Count(Formable)를 최우선으로 최적화하는 DLAS 핵심 루프"""
    states = np.empty((3, rows, cols), dtype=np.int64)
    states[0] = initial_grid.copy()
    states[1] = initial_grid.copy()
    states[2] = initial_grid.copy()

    # 여기서 scores는 'count' 값을 저장합니다.
    counts = np.array([initial_count, initial_count, initial_count], dtype=np.float64)
    cur_idx = 0
    best_idx = 0
    ...
    while idle_iter < max_idle:
        ...
        # 평가: 두 번째 값인 formable(count)을 사용
        _, new_count = _evaluate_core_numba(states[cand_idx].ravel(), rows, cols)

        # DLAS 수락 기준 (Count 기준)
        if new_count > counts[best_idx]:
            counts[cand_idx] = new_count
            best_idx = cur_idx = cand_idx
            idle_iter = 0
        elif new_count >= history[h_idx] or new_count >= counts[cur_idx]:
            counts[cand_idx] = new_count
            cur_idx = cand_idx
            idle_iter += 1
        else:
        ...
        if idle_iter == int(max_idle * 0.4):
            ...
            _, new_count = _evaluate_core_numba(states[cur_idx].ravel(), rows, cols)
            counts[cur_idx] = new_count

    return states[best_idx].ravel(), counts[best_idx]
...
def eval_814_heuristic(individual):
    ...
    return float(formable), float(current_score)  # Score first
...
def perform_dlas_count_local_search(pop):
    """Diversified Late Acceptance Search - Prioritizing formable count."""
    global GLOBAL_MAX_SCORE

    best_ind = tools.selBest(pop, 1)[0]
    initial_grid = np.array(best_ind).reshape(IND_ROWS, IND_COLS).copy()
    initial_count = float(best_ind.fitness.values[1])

    sys.stdout.write(f"\nDLAS (Count Focus) Started | Initial Count: {initial_count:.0f}\n")
    sys.stdout.flush()

    best_grid_1d, best_count = _dlas_count_core_numba(initial_grid, initial_count, 10000, IND_ROWS, IND_COLS)
    ...
    current_max_count = max([ind.fitness.values[1] for ind in pop])

    sys.stdout.write(f"DLAS Finished | Max Score: {current_max_score:.0f}, Max Count: {current_max_count:.0f}\n")
    sys.stdout.flush()

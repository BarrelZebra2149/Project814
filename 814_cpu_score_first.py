...
NGEN = 15000
G_PRINT_GROUP = 100
G_LOCAL_THRESHOLD = 20000
STAGNATION_LIMIT = 999999
ELITE_SIZE = 25
...
def custom_select(pop, stagnation_counter, nd_select, k, forbidden_items=None):
    ...
    if r < 0.40:
        ...
        candidates = tools.selNSGA2(pop, select_size)
    else:
...
MUTATION_TYPES = [
    structural_mutation, macro_mutation, structural_mutation_direction,
    cataclysmic_mutation, missing_hunter
]

NORMAL_PROBS = [0.05, 0.35, 0.40, 0.15, 0.05]
STAGNATION_PROBS = [0.08, 0.12, 0.30, 0.05, 0.30]
...
def load_individuals_from_file():
    ...
    if len(pop) < TARGET_POP:
        ...
        n = len(pop)
        top = pop[0: n // 5]
        upper_mid = pop[n // 5: 2 * n // 5]
        lower_mid = pop[2 * n // 5: 4 * n // 5]
        bottom = pop[4 * n // 5:]

        selected = []
        sections = [top, upper_mid, lower_mid, bottom]
        samples_per_section = TARGET_POP // 4

        for section in sections:
            num = min(samples_per_section, len(section))
            if num > 0:
                selected.extend(random.sample(section, num))

        while len(selected) < TARGET_POP:
            selected.append(random.choice(pop))

        random.shuffle(selected)
        pop = selected[:TARGET_POP]
    return pop


def analysis_file(pool, pop):
    """3. 병렬 풀 매핑 및 초기 인구 평가"""
    global GLOBAL_MAX_SCORE
...
def generation(g, pop, max_score_all_time, stagnation_counter, last_max):
    """4. 각 세대(Generation)별 진화 진행 로직"""
    global STAGNATION_MODE, GLOBAL_MAX_SCORE

    cxpb = CURRENT_CXPB
    mutpb = CURRENT_MUTPB

    best_set = tools.selBest(pop, 10)
    remaining_pool = [ind for ind in pop if ind not in best_set]
    nsga_rest = tools.selNSGA2(remaining_pool, ELITE_SIZE - 1)
    elites = list(map(toolbox.clone, best_set + nsga_rest))

    if STAGNATION_MODE:
        for e in elites:
            toolbox.mutate(e)
            del e.fitness.values
    ...
    return max_score_all_time, stagnation_counter, last_max


def save_result(pop, passed, g):
    """5. 최종 결과 저장"""
    sys.stdout.write("\n" + "=" * 60 + "\n")
    sys.stdout.write(f"Evolution finished after {g} generations.\n")
    sys.stdout.flush()

    if passed:
        print("\nGENERATION SUCCESS")
        top_k = tools.selBest(pop, k=200)
        with open(FILENAME, 'a') as f:
            sys.stdout.write(f"\n--- Final TOP 200 after {g} generations ---\n")
            for rank, ind in enumerate(top_k, 1):
                ...
                for row in grid:
                    f.write(''.join(map(str, row)) + '\n')
                f.write('\n')
        sys.stdout.write(f"Final TOP 200 saved to {FILENAME}\n")
        sys.stdout.flush()
    else:
...
def main():
    ...
    with multiprocessing.Pool() as pool:
        ...
        for g in range(1, NGEN + 1):
            max_score_all_time, stagnation_counter, last_max = generation(
                g, pop, max_score_all_time, stagnation_counter, last_max
            )

        # 5. Save Result
        save_result(pop, passed, NGEN)

...
def evaluate_batch_gpu(ind_lists):
    ...
    return formable_counts.cpu().numpy(), current_scores.cpu().numpy()
...
def main():
    ...
    forms, scores = evaluate_batch_gpu(ind_lists)  # ← swapped assignment
    for ind, f, s in zip(pop, forms, scores):
        ind.fitness.values = (f, s)

    update_crowding(pop)

    max_form_all_time = max([ind.fitness.values[0] for ind in pop]) if pop else -1.0
    sys.stdout.write(f"Initial Max Formable: {max_form_all_time:.0f}\n")
    sys.stdout.flush()
    ...
    for g in range(1, NGEN + 1):
        ...
        if invalid_ind:
            ...
            forms, scores = evaluate_batch_gpu(ind_lists)  # ← swapped
            for ind, f, s in zip(invalid_ind, forms, scores):
                ind.fitness.values = (f, s)

        pop[:] = offspring
        pop = update_crowding(pop)

        current_max = max([ind.fitness.values[0] for ind in pop])  # now formable

        if current_max > max_form_all_time:
            print(f"★ NEW RECORD! Gen {g}: {max_form_all_time:.0f} -> {current_max:.0f}")
            local_max_form = current_max
            max_form_all_time = current_max
        ...
        if g > G_LOCAL_THRESHOLD and current_max >= max_form_all_time * 0.8:
            if current_max > local_max_form:
                local_max_form = current_max
                local_max_ind = toolbox.clone(pop[np.argmax([ind.fitness.values[0] for ind in pop])])

        if g % G_PRINT_GROUP == 0:
            sys.stdout.write(f"Gen {g:5d} | Current Max Form: {current_max:.0f} "
                             f"| All-time Max Form: {max_form_all_time:.0f} "
                             f"| local max Form : {local_max_form:.0f}\n")

            sys.stdout.flush()

        if g > G_BOOM_THRESHOLD and local_max_form < 0:
            sys.stdout.write("\n" + "!" * 60 + "\n")

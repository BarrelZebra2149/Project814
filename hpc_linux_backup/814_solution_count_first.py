...
def eval_814_heuristic(individual):
    ...
    return float(formable_count), float(current_score)
...
def main():
    ...
    with multiprocessing.Pool() as pool:
        ...
        for g in range(1, NGEN + 1):
            ...
            pop[:] = offspring
            update_crowding(pop)
            pop_max_v = [ind.fitness.values[1] for ind in pop]
            current_max = max(pop_max_v)
            if current_max > best_current_score:
                best_current_score = current_max
                best_ind = max(pop, key=lambda ind: ind.fitness.values[1])
            # Stats Output
            if g % 500 == 0:
                formable = [ind.fitness.values[0] for ind in pop]
